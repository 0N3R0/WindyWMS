...
export function DashboardHeader() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-white/8 bg-white/2 backdrop-blur-md px-6 shadow-sm">
      <Link
        href="/dashboard"
        className="flex items-center gap-2 font-bold text-lg tracking-tight transition-colors text-violet-300 hover:text-violet-400 group"
      >
        <Package2 className="h-6 w-6" />
        <span>
          WindyWMS
          <span className="text-[.6rem] font-extrabold text-violet-400">&nbsp;DEMO</span>
        </span>
      </Link>

      <div className="ml-auto flex items-center gap-4">
        <div className="hidden md:flex items-center gap-2 text-sm text-violet-300/60 mr-4">
